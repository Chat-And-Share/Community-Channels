...
exports.run = async (bot, message, args, tools) => {
  ...
  if (!message.mentions.members.first() && !message.mentions.roles.first()) {
    const embed = new Discord.MessageEmbed().setTitle("Please mention a user or role following the command.");
  ...
  if (message.mentions.members.first().id === message.author.id || message.mentions.members.first().id === owner) {
    const embed = new Discord.MessageEmbed().setTitle("You cannot remove this user.");
  ...
  const embed = new Discord.MessageEmbed().setTitle(`Successfully removed ${target.name || target.user.username} from the channel.`);
