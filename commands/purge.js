...
exports.run = async (bot, message, args, tools) => {
  ...
  if (isNaN(args[0]) || args[0] < 0) {
    const embed = new Discord.MessageEmbed().setTitle("Please input a number following the command.");
  ...
  const embed = new Discord.MessageEmbed().setTitle(`Successfully deleted ${callback.size}/${args[0]} messages.`);
