...
exports.run = async (client, message, args, tools, forced) => {
  ...
  // Verify Parent
  let parent = message.guild.channels.cache.get(category);
  if (!parent || parent === null) category = null;

  let guildChannels = parent.children.array();
  for (var i in guildChannels) {
    const channelInfo = await db.fetch(`channelOwner_${guildChannels[i].id}`);
    if (channelInfo === null) continue;
    else if (channelInfo === message.author.id) {
      const embed = new Discord.MessageEmbed().setTitle(`Sorry, you already have a channel. Please ask an administrator if you would like a second channel.`);
  ...
  if (channel.name) {
    const first = new Discord.MessageEmbed().setTitle(`Channel created!`).setDescription(`Please make sure the bot has the proper permissions to view the channel. ${channel}`);
