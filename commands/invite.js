...
exports.run = async (client, message, args, tools) => {
  // This needs to be upgraded with Recursion

  let channelOwner = await db.fetch(`channelOwner_${message.channel.id}`),
    moderators = await db.fetch(`moderators_${message.channel.id}`),
    temporaryUser = client.users.cache.get(args.join(" "));

  if (moderators === null) moderators = [];

  if (channelOwner !== message.author.id && !moderators.includes(message.author.id) && !message.member.hasPermission("ADMINISTRATOR")) return tools.embed(message, "You do not have proper permissions to do this.");

  if (!args.join(" ")) return tools.embed(message, "Please specify a name of the user you would like to invite.");
  ...
  const embed = new Discord.MessageEmbed().setColor(0x1db954).setTitle(`Are you sure you would like to invite ${guildMember.user.username}?`).setThumbnail(guildMember.user.avatarURL());
  ...
  const acceptFilter = (reaction, user) => reaction.emoji.name === "✅" && user.id === message.author.id,
    accept = msg.createReactionCollector(acceptFilter, {
      time: 15000,
    }),
    rejectFilter = (reaction, user) => reaction.emoji.name === "⛔" && user.id === message.author.id,
    reject = msg.createReactionCollector(rejectFilter, {
      time: 15000,
    });
  ...
  reject.on("collect", (r) => {
    ...
    const resp = new Discord.MessageEmbed().setTitle(`The invitation for ${guildMember.user.username} has been revoked.`);
  ...
  accept.on("end", (r) => {
    if (!completed) embed.setDescription("**Timed out...**"), msg.edit(embed);
    msg.delete({
      timeout: 15000,
    });
  });
