...
exports.run = async (client, message, args, tools) => {
  ...
  if (!args[0] || !["true", "false", "true official"].includes(args.join(" ").toLowerCase())) {
    const embed = new Discord.MessageEmbed().setTitle('Please input "true", "true official", or "false" following the command.');
  ...
  switch (args.join(" ").toLowerCase()) {
    ...
    case "true":
      if (communityChannels.includes(message.channel.id)) {
        const embed = new Discord.MessageEmbed().setTitle("This channel is already listed as public.");
      ...
      await db.set(`communityChannels_${message.guild.id}`, [...communityChannels, message.channel.id]);

      const community = new Discord.MessageEmbed().setTitle("Successfully listed as a community channel.");
    ...
    case "true official":
      ...
      if (officialChannels.includes(message.channel.id)) {
        const embed = new Discord.MessageEmbed().setTitle("This channel is already listed as public under official.");
      ...
      const official = new Discord.MessageEmbed().setTitle("Successfully listed as an official channel.");
