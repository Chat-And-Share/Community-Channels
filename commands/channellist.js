...
exports.run = async (client, message, args, tools) => {
  async function createPosted() {
    ...
    const embed = new Discord.MessageEmbed().setTitle(`Loading Message...`).setColor(0xd7a489);

    let callback = await message.channel.send(embed);

    await db.set(`postedChannel_${message.guild.id}`, {
      enabled: true,
  ...
  if (enabled !== null && enabled.enabled) {
    ...
    channel = client.channels.cache.get(enabled.channel);
    ...
    console.log(msg);
    if (found) {
      const embed = new Discord.MessageEmbed().setDescription(`**Channel:** ${channel} | **MessageID:** ${msg.id} `).setFooter("This server already has a channel list.");
