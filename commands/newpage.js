...
exports.run = async (client, message, args, tools) => {
  if (!message.member.roles.find("name", "Editor")) {
    const embed = new Discord.MessageEmbed().setFooter("Sorry, you need the Editor role to create a webpage!");
  ...
  if (countInArray(authors, message.member.id) > 1) {
    const embed = new Discord.MessageEmbed().setFooter("Sorry, you already have 2 pages, please ask an administrator to `~assign` one to you.");
  ...
  let channel = await message.guild.channels
    ...
    .catch((e) => {
      ...
      client.guilds
        .get(guildID)
        .channels.create("Hypnospace-Community", {
          type: "category",
        })
        .then((i) => db.set(`category_${guildID}`, i.id));

      const embed = new Discord.MessageEmbed().setFooter("Created a new channel category, please re-run this command.");
  ...
  if (channel.name) {
    const first = new Discord.MessageEmbed().setFooter(`Welcome to Hypnospace!`);
