...
exports.run = async (client, message, args, tools) => {
  ...
  if (list === null || list.length === 0) {
    const embed = new Discord.MessageEmbed().setTitle("Sorry, there are no channels available.");
  ...
  if (isNaN(args[0])) {
    ...
    for (var i in fetched) {
      if (i === "pool");
        ...
        joining.push(i);
        try {
          final.push(client.channels.cache.get(i).name);
        } catch (e) {
          console.log(e.stack);
        }
      }
    ...
    if (joining.length === 0) {
      const embed = new Discord.MessageEmbed().setTitle("Please specify a number after the command.");
      ...
      for (var i in joining) {
        tools.addUserToChannel(message.author, client.channels.cache.get(joining[i]), false);
      }

      const embed = new Discord.MessageEmbed().setTitle(`Joining ${final.join(" & ")}...`);
  ...
  if (!list[args[0] - 1]) {
    const embed = new Discord.MessageEmbed().setTitle("Please specify a valid channel number.");
  ...
  let channel = client.channels.cache.get(list[args[0] - 1]);

  if (!channel) {
    const embed = new Discord.MessageEmbed().setTitle("Invalid channel, this channel may have been deleted.");
    ...
    const embed = new Discord.MessageEmbed().setTitle(`Joining ${channel.name}...`);
