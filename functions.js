...
module.exports = {
  // This basically works like every normal package you use.
  ...
  sendAd: async function (channel, del) {
    const init = new Promise(async (resolve) => {
      ...
      let callback = await send(channel, "_ _", {
        delete: del,
        files: [`../optional-channels/images/${index}.gif`],
        name: "Advertisement",
      ...
      const listCollector = callback.createReactionCollector(listFilter, {
        time: 30000,
      });
  ...
  updateChannels: async function (guild) {
    ...
    try {
      ...
      message = await guild.channels.cache.get(channel.channel).messages.fetch(channel.message);
    } catch (e) {
      // Message not found
      return;
    }
    ...
    else if (typeof officialChannels === "number") officialChannels = [officialChannels];
    else officialChannels = [...officialChannels];
    if (communityChannels === null) communityChannels = [];
    else if (typeof communityChannels === "number") communityChannels = [communityChannels];
    else communityChannels = [...communityChannels];
    ...
    if (index === 1) {
      embed.setTitle("No Channels Found").setDescription("**You can do *`~setpublic true`* \n**OR**\n *`~setpublic true official`* to get started!**");
    }
  ...
  findBestMatch: function (username, guild) {
    const getInfo = new Promise((resolve, error) => {
      ...
      resolve(guild.members.cache.get(indexes[users.indexOf(sm.findBestMatch(username, users).bestMatch.target)]));
    });
    return getInfo;
  },

  addUserToChannel: async function (user, channel, display) {
    let callback = await channel
      .updateOverwrite(user, {
        VIEW_CHANNEL: true,
      })
      .catch((e) => {
        const embed = new Discord.MessageEmbed().setTitle("This bot does not have the proper permissions (Manage Channels).");
    ...
    if (display && callback.name) {
      const resp = new Discord.MessageEmbed().setTitle(`${user.username} has been successfully invited.`);
  ...
  setChannels: function (message) {
    const embed = new Discord.MessageEmbed().setColor(0x1db954).setTitle("Optional Channels").setDescription("**Want to join a channel? Type:**\n`~join #`\n\n**To leave, type the following in the channel:**\n`~leave`");
  },

  setChannels: function (message) {
    // Someone needs to update this with Recursion
      ...
      db.fetchArray(`officialChannels_${message.guild.id}`).then((o) => {
        ...
        db.fetchArray(`publicChannels_${message.guild.id}`).then((p) => {
          ...
          message.guild.channels.cache
            .get("409922524948987924")
            .messages.fetch("412707175950057474")
